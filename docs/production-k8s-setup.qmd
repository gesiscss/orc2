...
Only on the master machine at `194.95.75.10`:
...
`--apiserver-advertise-address` is the IP address of the master machine (`194.95.75.10`).
...
sudo kubeadm init \
    ...
    --upload-certs \
    --control-plane-endpoint=194.95.75.10 \
...
----------- ----------- --------------------- -------
database    postgresql  PostgreSQL available  In use
binderhub   true        Can be used by Binder In use

Label the nodes to ensure that pods run in the desired node:

```bash
kubectl label nodes svko-css-app01 database=postgresql
```

```bash
kubectl label nodes spko-css-app03 binderhub=true
```
